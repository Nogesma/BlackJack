...
def score(player):  # Calculates the score of the player
    ...
    for card in range(len(player.cards)):
        # Takes the first character of each of the player's cards
        value = player.cards[card][0]
    ...
    # If the player has an ace but his first score is over 21, his hand become hard again
    if max(player.score) > 21:
        player.hand = "Hard"


def draw_card(deck, player):
    # Draw the first card of the deck and deletes it from the list
    player.cards.append(deck.pop(0))


def choose_bets(player):
    # The player can only bet between 2$ and his money
    while player.bet not in range(2, player.money + 1):
        player.bet = int(
            input(
                "{}, enter your bet, you have {}$: ".format(player.name, player.money)
            )
        )
...
def insurance(Players, Dealer):
    # Check if it's a case where the player can take insurance
    if not Dealer.cards[0][0].isdigit():
        for player in Players:
            # If the player's money is under half his bet, he can't take insurance
            if player.money < int(player.bet / 2):
                continue

            if (
                input("Do you wish to take insurance {} (y/n)?".format(player.name))
                == "y"
            ):
...
def further_turns(Players, Dealer, deck):  # Manage all the next turns
    for player in Players:
        ...
        print(
            "Your score is {}".format(
                player.score[0] if player.hand == "Hard" else player.score
            )
        )
    ...
    # Display the cards of the Dealer after all players have either gone bust or stand
    ...
    while dealer(Dealer):  # Check if the Dealer wants to hit or stand
        ...
        display_cards(Dealer)
        print(
            "Dealer's score is {}".format(
                Dealer.score[0] if Dealer.hand == "Hard" else Dealer.score
            )
        )
...
# Reset all the values that need to be reset for the next game
def reset_game(Players, Dealer):
    for player in Players:
        player.cards = []
        player.hand = "Hard"
